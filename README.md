...
| 观点研究 | 知乎先解析主页真实昵称并严格匹配本人回答/文章/想法（想法为富文本节点，含图片/视频占位，纯图想法不入库），雪球过滤点赞和纯转发、转评仅保留本人评论；保存原文版本，支持增量采集、**按时间回补历史**（回补任务可查进度/取消，已入库内容不重复分析）、全文补取、重试日志、观点共识、回测及博主可靠性统计 |
| 涨停板与情绪 | 精确涨跌停判定（主板 10% / 科创·创业 20% / 北交所 30% / ST 5%）、连板高度回溯、分时首次封板时间与炸板次数、板块梯队（申万行业 + 东财概念）与辨识度评分、情绪相位（冰点/启动/发酵/高潮/退潮）+ 晋级率与昨日溢价；直接驱动推荐里的「龙头 / 打板 / 补涨」风格与权重 |
